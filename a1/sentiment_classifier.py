...
def _parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="TRIVIAL",
        help="model to run (TRIVIAL, PERCEPTRON, or LR)",
    )
    parser.add_argument(
        "--feats",
        type=str,
        default="UNIGRAM",
        help="feats to use (UNIGRAM, BIGRAM, or BETTER)",
    )
    parser.add_argument(
        "--train_path",
        type=str,
...
def evaluate(classifier, exs):
    ...
    """
    print_evaluation(
        [ex.label for ex in exs], [classifier.predict(ex.words) for ex in exs]
    )
...
if __name__ == "__main__":
    ...
    if args.run_on_test:
        test_exs_predicted = [
            SentimentExample(words, model.predict(words))
            for words in test_exs_words_only
        ]
