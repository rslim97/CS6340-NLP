...
def read_sentiment_examples(infile: str, lowercase=True) -> List[SentimentExample]:
    ...
    for line in f:
        if len(line.strip()) > 0:
            ...
            if lowercase:
                sent = sent.lower()
            tokenized_cleaned_sent = list(
                filter(lambda x: x != "", sent.rstrip().split(" "))
            )
...
class WordEmbeddings:
    ...
    def get_initialized_embedding_layer(self, frozen=True):
        ...
        return torch.nn.Embedding.from_pretrained(
            torch.FloatTensor(self.vectors), freeze=frozen
        )
...
def relativize_sentiment_data():
    ...
    relativize(
        "data/glove.6B.50d.txt", "data/glove.6B.50d-relativized.txt", word_counter
    )
    relativize(
        "data/glove.6B.300d.txt", "data/glove.6B.300d-relativized.txt", word_counter
    )
...
if __name__ == "__main__":
    ...
    if embs.word_indexer.index_of(query_word_1) == -1:
        ...
        print(
            "cosine similarity of %s and %s: %f"
            % (
                query_word_1,
                query_word_2,
                np.dot(emb1, emb2) / np.sqrt(np.dot(emb1, emb1) * np.dot(emb2, emb2)),
            )
        )
