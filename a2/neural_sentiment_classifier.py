...
def _parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="DAN", help="model to run (TRIVIAL or DAN)"
    )
    parser.add_argument(
        "--train_path",
        type=str,
    ...
    parser.add_argument(
        "--word_vecs_path",
        type=str,
        default="data/glove.6B.300d-relativized.txt",
        help="path to word embeddings to use",
    )
    ...
    parser.add_argument(
        "--hidden_size", type=int, default=100, help="hidden layer size"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def evaluate(classifier, exs, has_typos):
    ...
    return print_evaluation(
        [ex.label for ex in exs],
        classifier.predict_all([ex.words for ex in exs], has_typos),
    )
...
if __name__ == "__main__":
    ...
    if args.model == "DAN":
        model = train_deep_averaging_network(
            args, train_exs, dev_exs, word_embeddings, args.use_typo_setting
        )
    else:
    ...
    if args.use_typo_setting:
        ...
        dev_typo_acc, dev_typo_f1, dev_typo_out = evaluate(
            model, dev_typo_exs, has_typos=True
        )
    ...
    if args.run_on_test:
        test_exs_predicted = [
            SentimentExample(words, model.predict(words, has_typos=False))
            for words in test_exs
        ]
    ...
    data = {
        "dev_acc": dev_acc,
        "dev_f1": dev_f1,
        "dev_typo_acc": dev_typo_acc,
        "dev_typo_f1": dev_typo_f1,
        "execution_time": train_eval_time,
        "output": dev_out,
        "typo_output": dev_typo_out,
    }
    print("=====Results=====")
