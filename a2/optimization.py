...
def _parse_args():
    ...
    parser.add_argument(
        "--func", type=str, default="QUAD", help="function to optimize (QUAD or NN)"
    )
...
def sgd_test_quadratic(args):
    ...
    plt.colorbar(cp)
    plt.plot(
        [p[0] for p in points_history],
        [p[1] for p in points_history],
        color="k",
        linestyle="-",
        linewidth=1,
        marker=".",
